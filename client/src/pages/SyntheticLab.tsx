...
const runCommand = `python3 ${syntheticUnicodeFacts.generatorPath} \\
  ...
  --profile unicode-clean --samples 12 --seed 10350`;
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <header className="grid gap-8 border-b border-[#ddd4c6] pb-10 lg:grid-cols-[1fr_0.7fr] lg:items-end">
          <div>
            ...
            <p className="mt-5 max-w-3xl text-sm leading-8 text-[#62675e]">مسار تدريجي لفهم بنية الكاشف قبل لمس المخطوطات الحقيقية: نبدأ بمحارف Unicode نظيفة، ثم نشوهها بصورة قابلة للقياس والإعادة، ثم لا ننتقل إلى الحبر التاريخي إلا بعد وسمه واعتماد حقوقه.</p>
          </div>
          <aside className="rounded-3xl border border-[#d9cfbf] bg-[#fffdf8] p-6 shadow-[0_12px_30px_rgba(70,60,44,0.06)]">
            <div className="grid grid-cols-3 divide-x divide-x-reverse divide-[#e9e0d4] text-center">
              ...
              <div><p className="text-2xl font-semibold text-[#2b4b40]">3</p><p className="mt-1 text-[11px] leading-5 text-[#746f65]">معاينات مولدة</p></div>
        ...
        <section className="mt-12"><div className="mb-6 flex items-center gap-3"><Sparkles className="size-5 text-[#a56b37]" /><div><p className="text-xs font-semibold tracking-[0.14em] text-[#a56b37]">STAGED DATA GENERATION</p><h2 className="mt-1 text-2xl font-semibold">المراحل المرئية</h2></div></div><div className="grid gap-6 xl:grid-cols-3">{syntheticStages.slice(0, 3).map((stage, index) => <article key={stage.id} className="overflow-hidden rounded-3xl border border-[#e0d8cb] bg-white shadow-[0_12px_30px_rgba(52,56,43,0.06)]"><div className="relative aspect-square bg-[#1f2922]"><img src={stage.previewUrl} alt={`معاينة ${stage.title}`} className="size-full object-contain" /><Badge className="absolute right-4 top-4 border-0 bg-[#2b4b40] text-[#fffaf0]">المرحلة {index + 1}</Badge></div><div className="p-6"><div className="flex items-start justify-between gap-4"><div><h3 className="text-xl font-semibold text-[#2b4b40]">{stage.title}</h3><p className="mt-2 text-sm leading-7 text-[#61675d]">{stage.goal}</p></div><CheckCircle2 className="mt-1 size-5 shrink-0 text-[#779067]" /></div><div className="mt-5 flex flex-wrap gap-2">{stage.controls.map((control) => <span key={control} className="rounded-full bg-[#f3efe6] px-3 py-1 text-[11px] font-medium text-[#686156]">{control}</span>)}</div><dl className="mt-5 grid gap-2 rounded-2xl bg-[#f7f9f4] p-4 text-xs leading-6 text-[#5b655b]"><div className="flex justify-between gap-4"><dt>Profile</dt><dd className="font-medium text-[#2b4b40]">{stage.profile}</dd></div><div className="flex justify-between gap-4"><dt>المعاينة</dt><dd className="font-medium text-[#2b4b40]">{stage.previewRun?.samples} عينة · seed {stage.previewRun?.seed}</dd></div></dl><p className="mt-4 border-t border-[#ece6da] pt-4 text-xs leading-6 text-[#7b7062]">بوابة المرحلة: {stage.gate}</p></div></article>)}</div></section>

        <section className="mt-14 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]"><article className="rounded-3xl border border-[#e0d8cb] bg-white p-7"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-[#f2e2c4] text-[#81572e]"><FileCode2 className="size-5" /></span><div><p className="text-xs font-semibold tracking-[0.15em] text-[#a56b37]">GENERATOR CONTRACT</p><h2 className="mt-1 text-xl font-semibold">ما الذي ينتجه الكود؟</h2></div></div><dl className="mt-6 divide-y divide-[#ede7da] text-sm"><div className="flex justify-between gap-4 py-3"><dt className="text-[#777168]">الخط</dt><dd className="font-medium text-[#2b4b40]">{syntheticUnicodeFacts.font}</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-[#777168]">الوسم</dt><dd className="font-medium text-[#2b4b40]">{syntheticUnicodeFacts.labelFormat}</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-[#777168]">الملفات</dt><dd className="font-medium text-[#2b4b40]">images · labels · class map · manifest</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-[#777168]">مصادر حقيقية</dt><dd className="font-medium text-[#a56b37]">غير مضمنة</dd></div></dl></article><article className="overflow-hidden rounded-3xl border border-[#273f35] bg-[#1d2c25] p-7 text-[#f7f1e4]"><div className="flex items-center gap-3"><Code2 className="size-5 text-[#d8b780]" /><div><p className="text-xs font-semibold tracking-[0.15em] text-[#d8b780]">REPRODUCIBLE PREVIEW</p><h2 className="mt-1 text-xl font-semibold">تشغيل معاينة المرحلة الأولى</h2></div></div><pre dir="ltr" className="mt-6 overflow-x-auto rounded-2xl border border-white/10 bg-black/20 p-5 text-left text-xs leading-7 text-[#f6ead4]"><code>{runCommand}</code></pre><p className="mt-5 text-sm leading-7 text-[#c9d1c5]">يسجل `manifest.json` البذرة والخط وخصائص التشويه. المخرج قابل لإعادة الإنتاج، لكنه ليس وزنًا مدرّبًا ولا مقياس دقة.</p></article></section>
