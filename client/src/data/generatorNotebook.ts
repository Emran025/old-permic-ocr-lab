export type GeneratorNotebookCell = {
  id: string;
  phase: "الأساس" | "S0" | "S1" | "S2" | "الحزمة" | "CLI";
  title: string;
  purpose: string;
  sourceRange: string;
  code: string;
...
const source = "training/synthetic/generate_old_permic_synthetic.py";

export const generatorNotebookSource = {
  path: source,
  githubUrl: "https://github.com/Emran025/old-permic-ocr-lab/blob/main/training/synthetic/generate_old_permic_synthetic.py",
  note: "تعرض هذه الصفحة كود Python من ملف المولد نفسه. لا ينفذ المتصفح الكود ولا يدّعي إنتاج بيانات جديدة؛ التعديل الفعلي يتم في الملف ثم يُراجع ويُولّد ويُتحقق منه كبصمة صغيرة مستقلة.",
} as const;

export const generatorNotebookCells: GeneratorNotebookCell[] = [
  {
    id: "foundation",
    phase: "الأساس",
    title: "استيراد الأدوات وملفات التشويه",
    purpose: "يثبت الخط المتاح، layouts المسموح بها، وملفات التشويه الثلاثة التي تستعملها المراحل اللاحقة.",
    sourceRange: "L1–41",
...
PROFILES: dict[str, Profile] = {
  ...
  },
  {
    id: "unicode-map",
    phase: "الأساس",
    title: "اشتقاق مخزون الحروف من Unicode",
    purpose: "ينشئ خريطة الفئات من 38 حرفًا برميًا أساسيًا، ولا يكتب قائمة حروف يدوية أو يتعامل مع كلمات.",
    sourceRange: "L52–62",
    code: String.raw`def old_permic_characters() -> list[dict[str, str]]:
  ...
  },
  {
    id: "s0",
    phase: "S0",
    title: "حرف مفرد موسوم",
    purpose: "يرسم حرف Unicode واحدًا ويحفظ صندوق YOLO واحدًا مطابقًا له؛ هذه بداية baseline الحرفي وليست مرحلة كلمات.",
    sourceRange: "L65–117",
    code: String.raw`def glyph_patch(glyph: str, font: ImageFont.FreeTypeFont, profile: Profile, rng: random.Random) -> Image.Image:
...
def render_isolated_glyph_sample(
  ...
  },
  {
    id: "s1",
    phase: "S1",
    title: "أسطر حروف منظمة",
    purpose: "يرتب محارف مستقلة داخل أسطر بصرية ويسجل صندوقًا لكل محرف. لا توجد مطالبة بأن التسلسل يمثل مفردات أو نصًا مقروءًا.",
    sourceRange: "L120–158",
    code: String.raw`def render_ordered_line_sample(
  ...
  },
  {
    id: "s2",
    phase: "S2",
    title: "صفحات صناعية منظمة",
    purpose: "ينشئ تخطيط صفحة ذي عمود واحد أو عمودين، ويحفظ ترتيب القراءة ومناطق الصفحة مع بقاء الوسم حرفيًا بصيغة YOLO.",
    sourceRange: "L161–231",
    code: String.raw`def render_structured_page_sample(
    characters: list[dict[str, str]], profile: Profile, seed: int, image_size: int, font_size: int, font_path: Path
) -> tuple[Image.Image, list[tuple[int, float, float, float, float]], list[str], dict[str, object]]:
  ...
  },
  {
    id: "package",
    phase: "الحزمة",
    title: "كتابة الصور والوسوم وسجل النسب",
    purpose: "يقسم البيانات إلى train/val/test، ويكتب وسوم YOLO، ثم يحفظ class_map وmanifest وassets.jsonl لمراجعة النسب قبل التدريب.",
    sourceRange: "L246–338 (المقطع الحاسم)",
    code: String.raw`def write_dataset(
    output_dir: Path, profile: Profile, samples: int, seed: int, image_size: int,
    font_size: int, font_path: Path, layout: str, balanced_classes: bool = False,
) -> dict[str, object]:
  ...
  },
  {
    id: "cli",
    phase: "CLI",
    title: "أمر تشغيل مضبوط",
    purpose: "يعرض المعاملات المتاحة ويطبع manifest فقط بعد نجاح التوليد. كل تغيير لاحق ينبغي أن يبدأ بتعديل صغير هنا أو في خلية مرحلة محددة ثم فحص الحزمة.",
    sourceRange: "L341–360",
...
export const incrementalWorkflow = [
  { id: "01", title: "اختيار متغير واحد", detail: "عدّل profile أو layout أو البذرة أو حجم العينة في موضع واحد فقط؛ لا تجمع عدة فرضيات في تشغيل واحد." },
  { id: "02", title: "توليد ثم تحقق", detail: "شغّل المولد، ثم validate_synthetic_dataset.py، وراجع manifest وassets.jsonl والمعاينات قبل بدء أي تدريب." },
  { id: "03", title: "تدريب محدود ومقاس", detail: "احفظ إعداد التدريب ونتيجة test المستقل وبصمة class_map مع كل checkpoint؛ لا تفسر نتيجة صناعية على أنها أداء مخطوطات." },
  { id: "04", title: "وزن موثوق واحد للواجهة", detail: "لا يُربط بواجهة الاختبار إلا آخر وزن اجتاز التقييم الموثق وخريطة الفئات المطابقة. أي وزن أحدث يبقى تجريبيًا حتى يثبت ذلك." },
] as const;
