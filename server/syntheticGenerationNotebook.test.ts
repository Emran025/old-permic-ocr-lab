...
describe("synthetic generation notebook", () => {
  it("keeps staged Jupyter cells wired to the real Python generator", () => {
    ...
    expect(code).toContain("cached_manifest_matches_generation");
    expect(code).toContain("cache البيانات المحلي يخص إعدادًا سابقًا");
    expect(code).toContain('for pattern in ("*.png", "*.jpg", "*.jpeg")');
    expect(code).toContain("خلل تطابق أسماء الصور والوسوم في {split}.");
